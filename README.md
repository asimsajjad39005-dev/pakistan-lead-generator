...
- **Smart Web Scraping:** Automatically visits extracted websites to hunt for `@` emails and `+92` / `03xx` phone numbers using advanced Regex patterns.
- **Pakistan-Optimized:** Hardcoded regional parameters (`gl: pk`) and strict city-name validation to ensure zero cross-border data leakage.
- **Intelligent Deduplication:** Automatically filters out duplicate business listings to provide a clean, unique lead list.
- **Bulletproof CSV Export:** Features built-in data sanitization to guarantee the output file opens perfectly in Excel without formatting errors.
